...
$$\hat{\mu}_y = \frac{1}{N}\sum_{i \in U} \hat{m}(\boldsymbol{x_i}) + \frac{1}{N}\sum_{i \in s} \frac{y_i - \hat{m}(\boldsymbol{x_i})}{\pi_i}$$
...
$$\hat{\mu}_y = \frac{1}{N}\sum_{i \in U} m(\boldsymbol{x_i}) + \frac{1}{N}\sum_{i \in s} \frac{y_i - m(\boldsymbol{x_i})}{\pi_i}$$
...
Is design unbiased: $E(\hat{\mu}_y) = \mu_y$

--

Has a simple variance equation:

$$\mbox{V}\left(\hat{\mu}_y\right) = \frac{1}{N^2}\underset{i, j \in U}{\sum \sum} (\pi_{ij} - \pi_i \pi_j) \frac{\left(y_i - m(\boldsymbol{x}_i)\right)}{\pi_i}\frac{\left(y_j - m(\boldsymbol{x}_j)\right)}{\pi_j}$$
...
$$\hat{\mbox{V}}\left(\hat{\mu}_y\right) = \frac{1}{N^2}\underset{i, j \in s}{\sum \sum} \frac{(\pi_{ij} - \pi_i \pi_j)}{\pi_{ij}} \frac{\left(y_i - m(\boldsymbol{x}_i)\right)}{\pi_i}\frac{\left(y_j - m(\boldsymbol{x}_j)\right)}{\pi_j}$$
...
Is .orange2[asymptotically] unbiased: $E(\hat{\mu}_y) \rightarrow \mu_y$
...
$$\hat{\mbox{V}}\left(\hat{\mu}_y\right) = \frac{1}{N^2}\underset{i, j \in s}{\sum \sum} \frac{(\pi_{ij} - \pi_i \pi_j)}{\pi_{ij}} \frac{\left(y_i - \hat{m}(\boldsymbol{x}_i)\right)}{\pi_i}\frac{\left(y_j - \hat{m}(\boldsymbol{x}_j)\right)}{\pi_j}$$
...
$$\hat{\mu}_y = \frac{1}{N}\sum_{i \in U} \boldsymbol{x}_i^T \hat{\boldsymbol{\beta}} + \frac{1}{N}\sum_{i \in s} \frac{y_i - \boldsymbol{x}_i^T \hat{\boldsymbol{\beta}}}{\pi_i}$$
...
\begin{align*}
\hat{\mu}_{y} &=  \frac{1}{N}\sum_{i \in s} \left[1 +  \left(\sum_{j \in U} \boldsymbol{x}_j - \sum_{j \in s} \frac{\boldsymbol{x}_j}{\pi_j}\right)^{T} \left( \sum_{j \in s} \frac{\boldsymbol{x}_j \boldsymbol{x}_j^{T}}{\pi_j} \right)^{-1} \boldsymbol{x}_i \right] \frac{y_i}{\pi_i}\\
&= \sum_{i \in s} w_i y_i
\end{align*}
...
knitr::include_graphics("img/nonresponse.jpg")
```

--

Determine a non-response model and account for it in the estimator.
...
1. Do nothing: Treat the respondents as the non-respondents.

--

2. Adjust the survey weights.
...
class: center, middle
background-image: url("img/donutfalls5.jpg")
background-position: left
background-size: contain
background-color: #CFF09E
