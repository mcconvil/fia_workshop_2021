...
knitr::include_graphics("img/tracey.jpg")
```
...
Horvitz-Thompson Estimator: $\hat{\mu}_y = \frac{1}{N}\sum_{i \in s} \frac{y_i}{\pi_i}$

--

For FIA's design: $\hat{\mu}_y = \frac{1}{n}\sum_{i \in s} y_i$
...
$\hat{\mu}_y = \frac{1}{N}\left[\sum_{i \in s} y_i + \sum_{i \in U-s}  \hat{m}(\boldsymbol{x_i})\right]$
...
Estimator: $\hat{\mu}_y = \frac{1}{N}\left[\sum_{i \in s} y_i + \sum_{i \in U-s}  \hat{m}(\boldsymbol{x_i})\right]$
...
$$\hat{\mu}_y = \frac{1}{N}\sum_{i \in U} \hat{m}(\boldsymbol{x_i}) + \frac{1}{n} \sum_{i \in s} y_i - \hat{m}(\boldsymbol{x_i})$$
...
---
class: center, middle
background-image: url("img/rain_montana.jpg")
background-size: contain
...
### What new auxiliary data do you wish to combine with FIA data to estimate forest attributes?
